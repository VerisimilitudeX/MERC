...
function Load-State {
    if (Test-Path $stateFile) {
        try {
            ...
            $script:downloadedFiles = $downloadedFiles.Count
...
function Log-Progress {
    ...
    $estimatedTimeRemaining = if ($downloadSpeed -gt 0 -and $script:downloadedFiles -gt 0) { 
        ($script:totalFiles - $script:downloadedFiles) / ($script:downloadedFiles / $elapsedTime) 
    } else { 
        0 
    }
