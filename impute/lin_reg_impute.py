...
models = {
    ...
    "Logistic Regression": LogisticRegression(max_iter=1000)  # for binarized prediction
}
...
for model_name, model in models.items():
    ...
    for fold, (train_index, validation_index) in enumerate(kf.split(X_train), 1):
        ...
        y_fold_train = y_train[train_index] if model_name != "Logistic Regression" else y_train_binary[train_index]
        y_fold_validation = y_train[validation_index] if model_name != "Logistic Regression" else y_train_binary[validation_index]
        ...
        print(f"{model_name} - Fold {fold} Training {'Accuracy' if model_name == 'Logistic Regression' else 'MSE'} = {train_metric}")
        print(f"{model_name} - Fold {fold} Validation {'Accuracy' if model_name == 'Logistic Regression' else 'MSE'} = {validation_metric}")

    # Calculate mean metrics
    mean_train_metric = np.mean(train_mse_list)
    mean_validation_metric = np.mean(validation_mse_list)

    print(f"{model_name} - Mean Training {'Accuracy' if model_name == 'Logistic Regression' else 'MSE'}: {mean_train_metric}")
    print(f"{model_name} - Mean Validation {'Accuracy' if model_name == 'Logistic Regression' else 'MSE'}: {mean_validation_metric}")
...
plt.plot(holdout_indices, original_h3k27ac_signal[holdout_indices], 'ro', label='Actual Hidden Values', markersize=5)

# Plot imputed values
plt.plot(holdout_indices, h3k27ac_signal[holdout_indices], 'go', label='Imputed Values', markersize=5)
